...

            {
                ...
                //Validamos la firma
                    ...
                    if ($order_id!="" && $ref_payco!="") {
                     ...
                     if (  !$ref_payco) {
                        $explode22=explode('?',$_GET['ref_payco']);
                        $ref_payco=$explode22[0];
                         $url = 'https://secure.epayco.co/validation/v1/reference/'.$ref_payco;
                            $response = wp_remote_get(  $url );
                            $body = wp_remote_retrieve_body( $response ); 
                            $jsonData = @json_decode($body, true);
                ...
                if (  !$ref_payco) {
                    ...
                    die();
                                                             ...
                                                             // var_dump("gateway",trim($validationData['x_signature']));
                    ...
                    if($signature == trim($validationData['x_signature'])){
                        switch ((int)trim($validationData['x_cod_response'])) {
                            case 1:{
...
// archivo
// hipervinculo 1)url confirmacion
//              2) url inteligente
